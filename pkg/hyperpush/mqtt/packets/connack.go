...
// NewConnackPacketFromErr return ConnackPacket with error code
func NewConnackPacketFromErr(err error) *ConnackPacket {
	packet := NewControlPacket(Connack).(*ConnackPacket)
	packet.ReturnCode = Accepted

	return packet
}

// NewConnackPacket return ConnackPacket with Accepted code
